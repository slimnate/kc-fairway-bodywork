<script lang="ts">
	...
	const postId = $derived($page.params.id as Id<'blogPosts'>);

	// Load post
	const postQuery = useQuery(api.blog.getPostById, () => ({ id: postId }));
	const post = $derived(postQuery?.data);
	const postLoading = $derived(postQuery?.isLoading);

	$inspect(postQuery);

	// Form state (populated from post)
	let title = $state('');
	let slug = $state('');
	let content = $state('');
	let excerpt = $state('');
	...
	$effect(() => {
		if (post) {
			...
			featuredImageStorageId = post.featuredImageStorageId;
			tags = [...post.tags];
			status = post.status;
			slugManuallyEdited = true; // Don't auto-update slug when editing

			// Load preview URL for featured image if it exists
			if (post.featuredImageStorageId) {
				convex
					.action(api.uploads.getPublicUrl, {
						id: post.featuredImageStorageId
					})
					.then(({ url }) => {
						featuredImagePreview = url ?? undefined;
					})
					.catch((err) => {
						console.error('Failed to load featured image preview:', err);
					});
			}
		}
	});
	...
	async function handleUpdate() {
		...
		try {
			await convex.mutation(api.blog.updatePost, {
				id: postId,
				title: title.trim(),
				slug: slug.trim(),
	...
	async function handleDelete() {
		try {
			await convex.mutation(api.blog.deletePost, { id: postId });
	...
	<div class="space-y-6">
		...
		<div class="flex items-end justify-between gap-4 px-2 pt-12 text-left md:px-4 md:pt-4">
			<div class="flex flex-col gap-2">
				<h1 class="text-primary text-3xl font-bold">Edit Blog Post</h1>
				<p class="text-base-content/60">Update your blog post content and settings</p>
			</div>
					...
					<div class="flex-1">
						<div class="mb-2 flex items-center gap-2">
							<div
								...
								data-tip="This feature costs money and should only be used after the post content is finalized. Make sure to save the post after generating so the result is not lost."
							>
								<button
									type="button"
